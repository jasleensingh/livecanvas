package livecanvas.thinplate.jama;
...
public class Matrix implements Cloneable, java.io.Serializable {
	...
	public static double[][] multiply(double[][] A, double[][] B) {
		int nArows = A.length;
		int nAcols = A[0].length;
		int nBrows = B.length;
		int nBcols = B[0].length;
		if (nAcols != nBrows) {
			throw new IllegalArgumentException(
					"Matrix inner dimensions must agree.");
		}
		double[][] C = new double[nArows][nBcols];
		double[] Bcolj = new double[nAcols];
	...
	public void print(int w, int d) {
		print(new PrintWriter(System.err, true), w, d);
	}
